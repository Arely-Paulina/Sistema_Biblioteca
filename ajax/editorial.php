	...
	case 'listar':
		$rspta=$editorial->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>($reg->condicion)?'<button class="btn btn-info" onclick="mostrar('.$reg->ideditorial.')"><i class="fas fa-edit"></i></button>'.
 					' <button class="btn btn-primary" onclick="desactivar('.$reg->ideditorial.')"><i class="fa fa-close"></i></button>':
 					'<button class="btn btn-info" onclick="mostrar('.$reg->ideditorial.')"><i class="fas fa-edit"></i></button>'.
 					' <button class="btn btn-primary" onclick="activar('.$reg->ideditorial.')"><i class="fa fa-check"></i></button>',
 				"1"=>$reg->nombre,
 				"2"=>$reg->descripcion,
 				"3"=>($reg->condicion)?'<span class="label bg-blue">Activado</span>':
